...
\item   Due to the challenges from utility constraints, we propose a novel mining framework, called UUH-Mine, which not only includes an efficient mining algorithm, called UUIM, but also contains an effective pruning technique.
...
In order to solve the MPHU problem, we propose a novel mining framework, called UUH-Mine, which is based on the divide-and-conquer framework and the depth-first search strategy.
...
In this data structure, each item is assigned with its label, its value, its appearing probability and a pointer. The UUH-Struct of Table 3 and Table 4 is shown in Fig. 1. After building the global UUH-Struct, the algorithm uses the depth-first strategy to build the head table in Fig. 2 where A is the prefix. Then, the algorithm recursively builds the head tables where different itemsets are prefix and generates all the itemsets in Fig. 3.
...
For example, the maximum expected utility of transaction 1 is 4.48, and the maximum expected utility of all 16 itemsets contained by transaction 1 is $\{A, B\}$, refer to Table 6.
...
In this example, the biggest transaction contains only 6 items, so we can calculate the maximum expected utility of every transaction through exhaustive method. However, in practical problems, a transaction may contains many items, so the exhaustive method is not efficient. Here we will introduce a faster way:

Given a transaction $T$ whose length (the number of item it contains) is $L$, items in $T$ are $\{i_1,i_2,...,i_L\}$, and the probability of each item is $\{p_1,p_2,...,p_L\}$. We can construct the sub problem $S_{X,j}$ (X represent an itemset), which means the maximum expected utility in set containing itemsets derived from itemset X and the last $j$ items in $T$. Obviously, we have $EU(T) = S_{\phi,L}$ and $S_{I,0} = EU(X,T)$ as well as a recursive relation:
...
\begin{theorem}
\label{the:SUPER}
If there exist itemsets $X_1$ and $X_2$ = $X_1 \cup \{i_j\}$ ($X_2$ represents a super-itemset which has one more item than $X_1$), and $EU(X_1,T) > EU(X_2,T)$, then expected utility of $X_2$ and all super-itemsets of $X_2$ cannot be the maximum expected utility of T.
\end{theorem}
\vspace{-0.2cm}
...
Due to $EU(X_1,T) > EU(X_2,T)$ and $P(X_1,T)\geq P(X_2,T)$, we can know $EU(X_3,T) < EU(X_4,T)$, i.e., the expected utility of $X_3$ cannot be the maximum expected utility of T.
\end{proof}
\vspace{-0.3cm}
...
When $esup(X) < \frac{min\_util}{U(X)}$, let $(1+ \delta) esup(X) = \frac{min\_util}{U(X)}$, so we can get $\delta = \frac{min\_util}{U(X)esup(X)}-1 = \frac{min\_util}{EU(X)} - 1$
...
Lemma 1 is the key point to solve the ¡°mining probabilistic high utility itemsets¡± problem in uncertain database, it can greatly reduce the search space so that the algorithm can be efficient.
...
\begin{algorithm}[htbp]
  ...
  \label{alg:uuim}
  \KwIn{an uncertain transaction database $ UD $, a utility threshold $ minutil $, a probabilistic utility threshold $ put $}
  \KwOut{a set of utility itemset $ UIS $}
...
\begin{algorithm}[htbp]
  ...
  \label{alg:recursion}
  \KwIn{a header table $ H_{I} $, an uncertain transaction database $ UD $, a utility threshold $ min\_util $, a probabilistic utility threshold $ put $, a current set of utility itemset $ UIS $}
  \KwOut{an updated set of utility itemset $ UIS $}
...
\vspace{-0.7cm}
  ...
  \subfigure[\small{Mushroom}]{
      \label{fig:mse:ol}
      \includegraphics[width=0.32\textwidth]{07.eps}}
  \subfigure[\small{Connect}]{
      \label{fig:mse:sf}
...
\vspace{-0.3cm}
  ...
  \subfigure[\small{Connect}]{
      \label{fig:error:sf}
      \includegraphics[width=0.32\textwidth]{11.eps}}
  \subfigure[\small{Accident}]{
...
\section{Conclusion}
\vspace{-0.3cm}
In this paper, we formulate a new type of problem of mining uncertain frequent itemsets, called \textit{\underline{m}ining \underline{p}robabilistic \underline{h}igh \underline{u}tility itemsets in uncertain databases} (MPHU), where each item has an unit value. In order to solve the \textit{MPHU} problem, we propose a novel mining framework, called \textit{UUH-Mime}, which not only includes an efficient mining algorithm but also contains an effective pruning technique. Extensive experiments on both real and synthetic datasets verify the effectiveness and efficiency of proposed solutions.
