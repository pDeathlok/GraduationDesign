...
\backslashbox[2cm]{Patient}{Incidence} & Apocleisis(A) & Drowsiness(B) & Fever(C) & Cold(D) & Gastritis(E) & Appendicitis(F) & Lung cancer(G) \\ \hline
...
This is the base idea of utility item set mining. Although this modeling idea can meet the demand of user more accurate, it's a difficult work for researchers.

\section{Definition}

\vspace{-0.3cm}

In this section, we first introduce some basic definitions and then define the problem of uncertain utility item set mining.
...
\textbf{Problem Statement}  Given a uncertain transaction database $ UD $, a user-specified minimum utility threshold minutil and a user-specified probabilistic utility threshold put, mining probabilistic utility itemset from the transaction database is equivalent to discover from $ UD $ all itemsets whose probabilistic utilities are no less than put.
...
In this section, we first introduce a new data mining framework UUH-mine; then we will describe several optimization approaches such as global bound and local pruning in order to make the algorithm efficient; finally, we will use the pseudo code and examples to describe the algorithm in detail.

\subsection{UUH-mine Framework}

We will use the table \ref{eoutd} and \ref{profit} to explain how UUH-mine framework works.
...
\begin{figure}[htbp]
  ...
  \vspace{-0.5cm}
  \caption{initial UUH-struct created from uncertain transaction database}\label{fig1}
  \vspace{-0.7cm}
\end{figure}
...
\caption{uncertain transaction database example after adding maximum transaction expected utility}
\label{utdeaamteu}
\end{table}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.99\textwidth]{4.PNG}\\
  \vspace{-0.5cm}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.99\textwidth]{5.PNG}\\
  \vspace{-0.5cm}
  \caption{Comparison of running time in different utility threshold}\label{fig5}
  \vspace{-0.7cm}
\end{figure}

Then we test the influence of the change of probabilistic utility threshold to UUIM algorithm and UUIM-NoCh algorithm. Fig. \ref{fig6} shows that while the probabilistic utility goes higher, the running time of UUIM-NoCh left largely unchanged, but the running time of UUIM algorithm is decreasing. Thus shows that while the probabilistic utility threshold gets higher, the effect of Chernoff bound pruning strategy will be better, but it doesn't influence UUIM-NoCh algorithm.

\vspace{-1cm}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.99\textwidth]{6.PNG}\\
  \vspace{-0.5cm}
  \caption{Comparison of running time in different utility threshold}\label{fig6}
  \vspace{-0.7cm}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.99\textwidth]{7.PNG}\\
  \vspace{-0.5cm}
  \caption{Comparison of running time in different utility threshold}\label{fig7}
  \vspace{-0.7cm}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.99\textwidth]{8.PNG}\\
  \vspace{-0.5cm}
  \caption{Comparison of memory cost in different probabilistic utility threshold}\label{fig8}
  \vspace{-0.7cm}
\end{figure}
...
In this paper we develop a new uncertain utility itemset mining algorithm, UUIM. This algorithm is based on the UUH-struct mining framework which can ensure traverse all itemset in mining progress. Meanwhile, we use Chernoff bound pruning strategy to reduce the search space. As shown in our performance study, UUIM has a high performance and has no influence on memory cost. Thus our UUIM algorithm can solve the uncertain utility itemset mining problem efficiently.

\begin{thebibliography}{4}

\bibitem{bib1} World Health Organization. World Health Statistics 2013 [M]. 2013:83-93

\bibitem{bib2} Mitzenmacher M., Upfal E.. Probability and computing. Cambridge, United Kingdom: The press syndicate of the university of Cambridge, 2005:61-8
